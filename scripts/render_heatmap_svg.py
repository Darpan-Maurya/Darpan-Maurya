...
def find_project_root():
    ...
    for candidate in candidates:
        if os.path.isdir(candidate):
            if os.path.isdir(os.path.join(candidate, "scripts")) and os.path.isdir(os.path.join(candidate, "data")):
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">darpan@github: ~/contributions --graph</text>')
